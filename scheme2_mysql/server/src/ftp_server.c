	...
	while(1)
	{
		...
		for(int i=0;i<ret;i++)
		{
			if(evs[i].data.fd==sfd)
			{
				...
				strcpy(temp,"用户登录选择1，用户注册选择2");
				send(new_fd,temp,sizeof(temp),0);
				recv(new_fd,&k,sizeof(int),0);
				if(1==k)
				{
					bzero(temp,sizeof(temp));
					recv(new_fd,temp,sizeof(temp),0);  //收到客户端发来的用户名
					...
					int n=query_name(temp,new_fd,&task[j]);
				...
				{
					char salt[512]={0};
					GenerateSalt(new_fd,salt);
					int ret=login_mysql(new_fd,salt,&task[j]);
					if(-1==ret)
						close(new_fd);
					else
					{
